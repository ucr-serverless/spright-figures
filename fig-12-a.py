...
# -------------- Parse raw data ----------------- #
# ----------------------------------------------- #
fileName_kn = ['parking/kn.parking_output.csv']
fileName_skmsg = ['parking/skmsg.parking_output.csv']
...
min_rests_kn = []
min_rests_skmsg = []
min_rests_kn.append(min(rest_1_ts))
min_rests_skmsg.append(min(rest_2_ts))

start_time_kn = float(int(min(min_rests_kn)))
start_time_skmsg = float(int(min(min_rests_skmsg)))

align_offset = 20
rest_1_ts = [x - start_time_kn + align_offset for x in rest_1_ts]
rest_2_ts = [x - start_time_skmsg + align_offset for x in rest_2_ts]
...
p1 = plt.plot(rest_1_ts, rest_1_d, marker = '3', ms = 10, mew = 2, label = '', linewidth=2.5, linestyle=" ", color='tab:green')
p2 = plt.plot(rest_2_ts, rest_2_d, marker = 'x', ms = 10, mew = 1, label = '', linewidth=2.5, linestyle=" ", color='#1F308D')
...
plt.ylim((0, 4))
plt.xlim((0, 700))
plt.yticks(np.arange(0, 3.1, 1), rotation = 45)
...
plt.tight_layout()
plt.savefig('fig-12-a.pdf', bbox_inches='tight', dpi=400, pad_inches=0.01)
