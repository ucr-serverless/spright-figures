...
fileName_grpc = ['online_boutique/grpc-front-prod.cpu', 'online_boutique/grpc-recom.cpu', 'online_boutique/grpc-others.cpu']
# fileName_kn = ['kn/' + concurrency + '/kn-front-prod.cpu', 'kn/' + concurrency + '/kn-recom.cpu', 'kn/' + concurrency + '/kn-others.cpu', 'kn/' + concurrency + '/kn-gw.cpu', 'kn/' + concurrency + '/kn-queue.cpu']
...
timestamps = [x for x in range(1, len(grpc_cpu_ts) + 1, 1)]
# print(len(timestamps))
p1 = plt.plot(timestamps, grpc_cpu_ts, marker = '', ms = 4, mew = 2, label = '', linewidth=2.5, linestyle="-", color='tab:blue')
...
plt.xlabel('(h) timestamp (second)', labelpad=0)

prop = dict(size=11)
ax.legend([p1[0]],['gRPC fn'],  loc = "upper left",
  ncol = 1,
  prop = prop,
  borderaxespad = 0,
  frameon = True,
...
plt.tight_layout()
plt.savefig('fig-10-h.pdf' ,bbox_inches='tight', dpi=400, pad_inches=0.01)
